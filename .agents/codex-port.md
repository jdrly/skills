...
- skill purpose, flow, and product behavior;
- skill frontmatter;
...
This branch owns only the Codex harness layer:
...
The reuse map in `$implement` is intentionally preserved from the initial local
import. It remains a separate design decision: do not expand, simplify, or
remove it while syncing upstream changes.
...
2. Re-express only the harness-specific mechanics for Codex.
...
4. Reconcile only the Codex harness layer listed above.
